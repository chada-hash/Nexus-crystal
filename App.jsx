    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0c0c28 0%,#0e0e2e 50%,#0a0a24 100%)",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 24px",boxSizing:"border-box",position:"relative",overflow:"hidden",touchAction:"pan-y",fontFamily:"Cinzel,Palatino,serif"}}>
      ...
      <div style={{width:"100%",maxWidth:440,padding:"8px 16px 0",boxSizing:"border-box",position:"relative",zIndex:10}}>
        <div style={{color:"rgba(220,200,160,.45)",fontSize:9,fontFamily:"Georgia,serif",fontStyle:"italic",lineHeight:1.5,textAlign:"center"}}>
          {tab==="cristal"&&"Écris un mot ci-dessous, puis Décrypte-le : tu verras sa valeur, son archétype et les Noms divins qui résonnent avec lui."}
          {tab==="racines"&&"Tape 3 lettres : le Cristal montre les 6 permutations de la racine, toutes de même valeur. La preuve que l'arabe est mathématique."}
          {tab==="inversion"&&"Le miroir révèle comment les mêmes lettres peuvent servir la Lumière ou les ténèbres, selon l'intention qui les anime."}
          {tab==="awraq"&&"Chaque lettre est reliée à un Nom divin, un aliment de la Sunna et une sourate, sources du Coran et du Hadith."}
        </div>
      </div>
      {/* ── CRISTAL ── */}
      {tab==="cristal"&&(
        ...
        <div style={{width:"100%",maxWidth:440,padding:"16px 16px",boxSizing:"border-box"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:4}}>
            <input value={rootIn} onChange={e=>setRootIn(e.target.value)}
              placeholder="r-h-m · س-ل-م · 3 lettres FR ou arabe"
              style={{flex:1,background:"rgba(8,6,22,.9)",border:"none",borderBottom:"1px solid "+D,color:G,fontSize:18,padding:"8px 4px",fontFamily:[...rootIn].some(c=>ABJAD[c])?"Amiri,serif":"Cinzel,serif",direction:[...rootIn].some(c=>ABJAD[c])?"rtl":"ltr",textAlign:[...rootIn].some(c=>ABJAD[c])?"right":"left",borderRadius:0}}/>
          </div>
          {rootIn&&(
