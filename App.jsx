...
const FR2AR_WORDS={
  "allah":"الله","bismillah":"بسم الله","allahou":"الله",
  "salam":"سلام","salaam":"سلام","islam":"إسلام",
  "rahman":"رحمن","rahim":"رحيم","karim":"كريم",
  "nour":"نور","nur":"نور","noor":"نور",
  "mahdi":"مهدي","muhammad":"محمد","ahmed":"أحمد",
  "ibrahim":"إبراهيم","moussa":"موسى","issa":"عيسى",
  "quran":"قرآن","coran":"قرآن","sourate":"سورة",
  "haq":"حق","haqq":"حق","barakah":"بركة",
  "tawbah":"توبة","sabr":"صبر","shukr":"شكر",
  "fatiha":"فاتحة","ikhlas":"إخلاص",
};
...
export default function NEXUSCrystal() {
  ...
  const handleKun = useCallback(async()=>{
    ...
    const q=inputMode==="fr"?frToAr(raw).replace(/\s/g,'').replace(/[^\u0600-\u06ff]/g,''):raw;
    if(!q) return;
    ...
    setAnalysis({letters,total,reduced,constellations,original:raw});
      ...
      const res=await fetch("/api/oracle",{
        ...
        body:JSON.stringify({
          ...
          messages:[{role:"user",content:
            ...
            "Réponds en 3 langues — pour chaque point écris d'abord en français, puis en anglais, puis en arabe.\n"+
            "Style : oraculaire, précis. Maximum 9 lignes total (3 par langue)."
          }]
        })
      });
  ...
  if(splash) return(
    <div style={{minHeight:"100vh",background:"radial-gradient(ellipse at 50% 35%,#13123a 0%,#0a0a24 55%,#050514 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",boxSizing:"border-box",position:"relative",overflow:"hidden",fontFamily:"Cinzel,Palatino,serif"}}>
      ...
      <div className="fadein" style={{textAlign:"center",maxWidth:380,position:"relative",zIndex:10}}>
        <div style={{color:G,fontSize:26,fontFamily:"Amiri,serif",direction:"rtl",lineHeight:1.9,filter:"drop-shadow(0 0 16px rgba(232,201,122,.5))",marginBottom:6}}>Ø¨ÙØ³ÙÙÙ Ø§ÙÙÙÙ Ø§ÙØ±ÙÙØ­ÙÙÙÙ°ÙÙ Ø§ÙØ±ÙÙØ­ÙÙÙÙ</div>
        <div style={{color:"rgba(232,201,122,.4)",fontSize:8,letterSpacing:7,marginBottom:38}}>N E X U S · C R I S T A L · D E · H U</div>
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0c0c28 0%,#0e0e2e 50%,#0a0a24 100%)",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 24px",boxSizing:"border-box",position:"relative",overflow:"hidden",touchAction:"pan-y",fontFamily:"Cinzel,Palatino,serif"}}>
      ...
      <div style={{width:"100%",maxWidth:440,padding:"14px 16px 0",boxSizing:"border-box",position:"relative",zIndex:10}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          {mansion&&(
            <div style={{padding:"4px 8px",border:"1px solid rgba(180,130,255,.25)",borderRadius:1,background:"rgba(180,130,255,.06)",display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <div style={{flex:1}}>
                <div style={{color:"rgba(200,160,255,.45)",fontSize:6,letterSpacing:2}}>{"MANZEL AL-QAMAR · "+(mansion.index+1)+"/28"}</div>
                <div style={{color:"rgba(200,160,255,.75)",fontSize:8,marginTop:1}}>{mansion.name}</div>
                <div style={{color:"rgba(200,160,255,.3)",fontSize:6}}>{mansion.star}</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:24,color:"rgba(200,160,255,.9)",fontFamily:"Amiri,serif",filter:"drop-shadow(0 0 8px rgba(180,130,255,.5))"}}>{mansion.letter}</div>
                <div style={{color:"rgba(200,160,255,.35)",fontSize:6}}>{mansion.longitude+"°"}</div>
              </div>
            </div>
          )}
      ...
      <div style={{width:"100%",maxWidth:440,padding:"8px 16px 0",boxSizing:"border-box",position:"relative",zIndex:10}}>
        <div style={{color:"rgba(220,200,160,.45)",fontSize:9,fontFamily:"Georgia,serif",fontStyle:"italic",lineHeight:1.5,textAlign:"center"}}>
          {tab==="cristal"&&"Ãcris un mot ci-dessous, puis DÃ©crypte-le : tu verras sa valeur, son archÃ©type et les Noms divins qui rÃ©sonnent avec lui."}
          {tab==="racines"&&"Tape 3 lettres : le Cristal montre les 6 permutations de la racine, toutes de mÃªme valeur. La preuve que l'arabe est mathÃ©matique."}
          {tab==="inversion"&&"Le miroir rÃ©vÃ¨le comment les mÃªmes lettres peuvent servir la LumiÃ¨re ou les tÃ©nÃ¨bres, selon l'intention qui les anime."}
          {tab==="awraq"&&"Chaque lettre est reliÃ©e Ã  un Nom divin, un aliment de la Sunna et une sourate, sources du Coran et du Hadith."}
        </div>
      </div>
      {/* ── CRISTAL ── */}
      {tab==="cristal"&&(
        ...
        <div style={{width:"100%",maxWidth:440,padding:"16px 16px",boxSizing:"border-box"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:4}}>
            <input value={rootIn} onChange={e=>setRootIn(e.target.value)}
              placeholder="r-h-m · s-l-m · n-w-r · 3 lettres FR"
              style={{flex:1,background:"rgba(8,6,22,.9)",border:"none",borderBottom:"1px solid "+D,color:G,fontSize:18,padding:"8px 4px",fontFamily:"Cinzel,serif",direction:"ltr",textAlign:"left",borderRadius:0}}/>
          </div>
          {rootIn&&(
