...
.seal-3d{transition:transform .05s linear;will-change:transform}
...
export default function NEXUSCrystal() {
  ...
  const [rot3d,setRot3d]       = useState({x:-8,y:0});
  ...
  useEffect(()=>{
    try{
      ...
      socket.on("collective_pulse",({word,count})=>{setNafarCount(count);setNafarPulse(true);setTimeout(()=>setNafarPulse(false),3000);});
  ...
  const on3dDown = useCallback(e=>{
    ...
    drag3d.current={on:true,lx:src.clientX,ly:src.clientY,rx:rot3d.x,ry:rot3d.y};
    e.preventDefault();
  },[rot3d]);
  ...
  const on3dMove = useCallback(e=>{
    ...
    setRot3d({x:drag3d.current.rx-dy*.35, y:drag3d.current.ry+dx*.35});
    setHov(null);
    e.preventDefault();
  },[]);
  ...
  const sealTransform=`perspective(900px) rotateX(${rot3d.x}deg) rotateY(${rot3d.y}deg) scale(${zoom})`;

  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#01010d 0%,#020215 50%,#010112 100%)",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 24px",boxSizing:"border-box",position:"relative",overflow:"hidden",fontFamily:"Cinzel,Palatino,serif"}}>
      <style>{CSS}</style>
      ...
      {tab==="cristal"&&(
        <div style={{width:"100%",maxWidth:440,display:"flex",flexDirection:"column",alignItems:"center",position:"relative",zIndex:2}}>
          <div ref={wrapRef} style={{perspective:"900px",marginTop:8,cursor:"grab",userSelect:"none"}}
            ...
            <div className="seal-3d" style={{transform:sealTransform}}>
              <SealSVG inv={false}/>
            </div>
          </div>
      ...
      {tab==="inversion"&&(
        <div style={{width:"100%",maxWidth:440,display:"flex",flexDirection:"column",alignItems:"center"}}>
          ...
          <div style={{perspective:"900px"}}>
            <div style={{transform:`perspective(900px) rotateX(-8deg) rotateY(${rot3d.y}deg) scale(${zoom})`}}>
              <SealSVG inv={true}/>
            </div>
          </div>
