...
export default function NEXUSCrystal() {
  ...
  const fetchQuranRoot = useCallback(async(rootStr)=>{
    ...
    try{
      ...
      setQuranV(results.slice(0,6).map(v=>({
        key:v.verse_key,
        ar:v.text||"",
        fr:(v.translations&&v.translations[0]&&v.translations[0].text)||""
      })));
    }catch(e){ setQuranV([]); }
    setQuranLoad(false);
  },[]);
  ...
  const handleKun = useCallback(async()=>{
    ...
    const trusted=inputMode==="ar"||isFrTrusted(raw);
    setAnalysis({letters,total,reduced,constellations,original:raw,trusted,wasFr:inputMode==="fr"});
              ...
              {quranV&&(
                <div className="fadein" style={{marginTop:10}}>
                  {quranV.length===0?(
                    ...
                    <>
                      <div style={{color:"rgba(232,201,122,.4)",fontSize:7,letterSpacing:2,marginBottom:6,textAlign:"center"}}>OCCURRENCES CORANIQUES (source vérifiée)</div>
                      {quranV.map((v,i)=>(
                        <div key={i} style={{padding:"8px 10px",border:"1px solid rgba(232,201,122,.1)",borderRadius:3,background:"rgba(232,201,122,.02)",marginBottom:6}}>
                          <div style={{color:G,fontSize:14,fontFamily:"Amiri,serif",direction:"rtl",textAlign:"right",lineHeight:1.8}} dangerouslySetInnerHTML={{__html:v.ar}}/>
                          <div style={{color:"rgba(220,200,160,.6)",fontSize:8.5,fontFamily:"Georgia,serif",fontStyle:"italic",marginTop:4,lineHeight:1.5}} dangerouslySetInnerHTML={{__html:v.fr}}/>
