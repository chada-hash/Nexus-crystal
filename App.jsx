...
export default function NEXUSCrystal() {
  ...
  const drag3d                 = useRef({on:false,lx:0,ly:0,rx:0,ry:0});
  const pinch                  = useRef({on:false,d0:0,z0:1});
  const wrapRef                = useRef(null);

  useEffect(()=>{
    try{const socket=io(window.location.origin,{transports:["websocket","polling"]});socketRef.current=socket;socket.on("collective_pulse",({count})=>{setNafarCount(count);});socket.on("room_count",({count})=>setNafarCount(count));return()=>socket.disconnect();}catch(e){}
  },[]);
  ...
  const applyTransform = useCallback(()=>{
    const {x,y}=rot3dRef.current;
    const t=`perspective(900px) rotateX(${x}deg) rotateY(${y}deg) scale(${zoom})`;
    if(sealWrapRef.current) sealWrapRef.current.style.transform=t;
    if(invWrapRef.current) invWrapRef.current.style.transform=`perspective(900px) rotateX(${x}deg) rotateY(${y}deg) scale(${zoom})`;
  },[zoom]);

  const on3dDown = useCallback(e=>{
    const src=e.touches?e.touches[0]:e;
    drag3d.current={on:true,lx:src.clientX,ly:src.clientY,rx:rot3dRef.current.x,ry:rot3dRef.current.y};
    e.preventDefault();
  },[]);

  const on3dMove = useCallback(e=>{
    if(!drag3d.current.on) return;
    const src=e.touches?e.touches[0]:e;
    const dx=src.clientX-drag3d.current.lx;
    const dy=src.clientY-drag3d.current.ly;
    rot3dRef.current={x:drag3d.current.rx-dy*.4, y:drag3d.current.ry+dx*.4};
    applyTransform();
    setHov(null);
    e.preventDefault();
  },[applyTransform]);

  const on3dUp = useCallback(()=>{ drag3d.current.on=false; },[]);
  ...
  const onTouchMove = useCallback(e=>{
    if(pinch.current.on&&e.touches.length===2){
      const d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);
      const newZ=Math.min(2.5,Math.max(.5,pinch.current.z0*(d/pinch.current.d0)));
      pinch.current.z0=newZ/( d/pinch.current.d0); // update z0
      if(sealWrapRef.current) sealWrapRef.current.style.transform=`perspective(900px) rotateX(${rot3dRef.current.x}deg) rotateY(${rot3dRef.current.y}deg) scale(${newZ})`;
      setZoom(newZ);
      e.preventDefault();
    } else { on3dMove(e); }
  },[on3dMove]);

  const onTouchEnd = useCallback(e=>{ pinch.current.on=false; on3dUp(); },[on3dUp]);

  // ── scroll zoom ──
  const onWheel = useCallback(e=>{
    setZoom(z=>{
      const nz=Math.min(2.5,Math.max(.5,z-e.deltaY*.001));
      if(sealWrapRef.current) sealWrapRef.current.style.transform=`perspective(900px) rotateX(${rot3dRef.current.x}deg) rotateY(${rot3dRef.current.y}deg) scale(${nz})`;
      return nz;
    });
    e.preventDefault();
  },[]);
  ...
  function SealSVG({inv=false}){
    ...
    return(
      <svg width={S} height={S} viewBox={"0 0 "+S+" "+S}
        style={{maxWidth:"92vw",maxHeight:"45vh",display:"block",cursor:"grab",touchAction:"none"}}
        onMouseDown={on3dDown} onMouseMove={on3dMove} onMouseUp={on3dUp} onMouseLeave={on3dUp}>
        <defs>
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0c0c28 0%,#0e0e2e 50%,#0a0a24 100%)",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 24px",boxSizing:"border-box",position:"relative",overflow:"hidden",fontFamily:"Cinzel,Palatino,serif"}}>
      <style>{CSS}</style>
      ...
      {tab==="cristal"&&(
        <div style={{width:"100%",maxWidth:440,display:"flex",flexDirection:"column",alignItems:"center",position:"relative",zIndex:2}}>
          <div ref={wrapRef} style={{perspective:"900px",marginTop:8,cursor:"grab",userSelect:"none"}}
            onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            <div ref={sealWrapRef} className="seal-3d" style={{transform:`perspective(900px) rotateX(${rot3dRef.current.x}deg) rotateY(0deg) scale(${zoom})`}}>
              <SealSVG inv={false}/>
            </div>
          </div>
          ...
          <div style={{width:"100%",maxWidth:380,padding:"0 14px",boxSizing:"border-box"}}>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleKun()}
                ...
                style={{flex:1,background:"rgba(8,6,22,.9)",border:"none",borderBottom:"1px solid "+D,color:G,fontSize:20,padding:"8px 4px",fontFamily:"Amiri,serif",direction:"rtl",textAlign:"right",borderRadius:0}}/>
      ...
      {tab==="inversion"&&(
        <div style={{width:"100%",maxWidth:440,display:"flex",flexDirection:"column",alignItems:"center"}}>
          ...
          <div style={{perspective:"900px"}}>
            <div ref={invWrapRef} className="seal-3d" style={{transform:`perspective(900px) rotateX(-8deg) rotateY(0deg) scale(${zoom})`}}>
              <SealSVG inv={true}/>
            </div>
          </div>
