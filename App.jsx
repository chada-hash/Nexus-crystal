...
const S=380,CX=190,CY=190,R=130,LR=163;
...
export default function NEXUSCrystal() {
  ...
  const [kunStep,setKunStep] = useState(0); // 0=off 1-5=guided steps
  ...
  useEffect(() => {
    ...
    socket.on("collective_pulse",({word,count})=>{setNafarCount(count);setNafarWord(word||"");setNafarPulse(true);setTimeout(()=>setNafarPulse(false),3000);});
    socket.on("room_count",({count})=>setNafarCount(count));
    const params=new URLSearchParams(window.location.search);
    const kp=params.get("kun"); if(kp) setInput(decodeURIComponent(kp));
  ...
  const startGuided = useCallback((q) => {
    if(!q) return;
    setKunStep(1);
    let step=1;
    const durations=[0,3000,3000,2500,2500,2000];
    function next(){
      step++;
      if(step>5){ setKunStep(0); fireKun(q); return; }
      setKunStep(step);
      kunTimer.current=setTimeout(next, durations[step]);
    }
    kunTimer.current=setTimeout(next, durations[1]);
  },[]);

  const fireKun = useCallback(async (q) => {
    ...
    const result={letters,total,reduced,constellations};
            ...
            {ALL.map(letter => {
              ...
              return (
                <g key={letter.deg}
                  ...
                  {/* valeur abjad — TOUJOURS visible, toutes les 28 lettres */}
                  <text x={lx} y={ly+fs/2+9} textAnchor="middle"
                    transform={iTx} fontSize={isV?"7.5":"5.5"} fill={col}
                    opacity={isH?1:isV?.85:.65}
                    style={{fontFamily:"Cinzel,serif"}}>
                    {letter.v}
                  </text>
                  {/* phonétique française — TOUJOURS visible, toutes les 28 lettres */}
                  <text x={lx} y={ly-fs/2-8} textAnchor="middle"
                    transform={iTx} fontSize={isV?"7":"5"} fill={col}
                    opacity={isH?1:isV?.75:.55}
    ...
    <div style={{minHeight:"100vh",background:"#020210",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 20px",boxSizing:"border-box",position:"relative",overflow:"hidden",fontFamily:"Cinzel,Palatino,serif"}}>
      ...
      <div style={{width:"100%",maxWidth:420,padding:"14px 14px 0",boxSizing:"border-box",position:"relative",zIndex:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{color:"#C9A84C",fontSize:15,fontFamily:"Amiri,serif",direction:"rtl",textAlign:"right",letterSpacing:1,filter:"drop-shadow(0 0 10px rgba(200,160,50,.5))",marginBottom:4}}>بِسْمِ اللهِ الرَّحْمَٰنِ الرَّحِيمِ</div>
            <div style={{color:"rgba(200,165,60,.45)",fontSize:7,letterSpacing:5,marginTop:2}}>N E X U S</div>
            <div style={{color:"#C9A84C",fontSize:12,letterSpacing:3,marginTop:2,fontWeight:600,filter:"drop-shadow(0 0 8px rgba(200,160,50,.4))"}}>Cristal de HU · هُوَ</div>
            <div style={{color:"rgba(200,160,50,.35)",fontSize:7,marginTop:2,fontFamily:"Amiri,serif",direction:"rtl"}}>
                ...
                {analysis.letters.length===0 ? (
                  ...
                  <div>
                    ...
                    {analysis.constellations && analysis.constellations.length>0 && (
                      <div style={{borderTop:"1px solid rgba(200,160,50,.1)",paddingTop:7,marginBottom:6}}>
                        <div style={{color:"rgba(200,160,50,.38)",fontSize:6.5,letterSpacing:2,marginBottom:4}}>CONSTELLATIONS DÉTECTÉES</div>
                        {analysis.constellations.map((con,i)=>(
                          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 6px",marginBottom:3,border:"1px solid "+(con.pirate?"rgba(180,50,50,.25)":"rgba(200,160,50,.15)"),borderRadius:1,background:con.pirate?"rgba(20,0,0,.4)":"rgba(200,160,50,.04)"}}>
                            <div>
                              <span style={{color:con.pirate?"#8B1A1A":"#C9A84C",fontFamily:"Amiri,serif",fontSize:14}}>{con.l.join("-")}</span>
                              <span style={{color:con.pirate?"rgba(180,50,50,.6)":"rgba(200,160,50,.5)",fontSize:7,marginLeft:6}}>{con.id}</span>
                            </div>
                            <div style={{color:con.pirate?"rgba(180,50,50,.7)":"rgba(220,194,136,.75)",fontSize:8,textAlign:"right",maxWidth:"55%"}}>{con.fr}</div>
                          </div>
                        ))}
                      </div>
                    )}
      ...
      {kunStep>0 && (
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(2,2,16,.97)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:32}}>
            {[1,2,3,4,5].map(i=>(
              <div key={i} style={{width:6,height:6,borderRadius:"50%",background:kunStep>=i?"#C9A84C":"rgba(200,160,50,.2)",transition:"background .3s",boxShadow:kunStep===i?"0 0 8px #C9A84C":undefined}}/>
            ))}
          </div>

          {kunStep===1 && (
            <div style={{textAlign:"center",animation:"fadeUp .4s ease both"}}>
              <div style={{fontSize:64,color:"#C9A84C",fontFamily:"Amiri,serif",filter:"drop-shadow(0 0 20px rgba(200,160,50,.6))",marginBottom:16}}>نِيَّة</div>
              <div style={{color:"#C9A84C",fontSize:11,letterSpacing:3,marginBottom:8}}>NIYYA · INTENTION</div>
              <div style={{color:"rgba(200,160,50,.6)",fontSize:9,lineHeight:1.9,maxWidth:280}}>Purifie ton cœur.<br/>Pour qui, pour quoi invoques-tu ce Verbe ?<br/>Que ton intention soit pure devant Allâh.</div>
              <div style={{marginTop:16,padding:"8px 16px",border:"1px solid rgba(200,160,50,.2)",borderRadius:2,background:"rgba(200,160,50,.05)",color:"rgba(200,160,50,.5)",fontSize:8,lineHeight:1.8,maxWidth:260,fontFamily:"Georgia,serif",fontStyle:"italic"}}>
          ...
          {kunStep===2 && (
            <div style={{textAlign:"center",animation:"fadeUp .4s ease both"}}>
              <div style={{width:80,height:80,borderRadius:"50%",background:"radial-gradient(circle,white,#80b4ff,transparent)",margin:"0 auto 20px",animation:"kunPulse 1s ease-in-out infinite",filter:"drop-shadow(0 0 30px rgba(100,160,255,.8))"}}/>
              <div style={{fontSize:48,color:"rgba(100,180,255,.9)",fontFamily:"Amiri,serif",marginBottom:12}}>نُون</div>
              <div style={{color:"rgba(200,160,50,.6)",fontSize:9,lineHeight:1.9}}>Visualise le Point Primordial.<br/>Il est au centre de tout — en toi, dans l'univers.<br/>Il pulse. Tu pulses. Nous pulsons.</div>
            </div>
          )}
          {kunStep===3 && (
            <div style={{textAlign:"center",animation:"fadeUp .4s ease both"}}>
              <div style={{fontSize:52,color:"#C9A84C",fontFamily:"Amiri,serif",filter:"drop-shadow(0 0 16px rgba(200,160,50,.7))",marginBottom:12}}>اسم</div>
              <div style={{color:"#C9A84C",fontSize:10,letterSpacing:3,marginBottom:16}}>LE NOM DIVIN RÉSONNANT</div>
              {analysis && (() => {
                const matches=N99.map(n=>({...n,red:reduce(abj(n.ar))})).filter(n=>n.red===analysis.reduced).slice(0,3);
                return matches.length ? (
                  <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
                    {matches.map((n,i)=>(
                      <div key={i} style={{textAlign:"center",padding:"8px 14px",border:"1px solid rgba(200,160,50,.3)",borderRadius:2,background:"rgba(200,160,50,.06)"}}>
                        <div style={{fontSize:28,color:"#C9A84C",fontFamily:"Amiri,serif"}}>{n.ar}</div>
                        <div style={{color:"rgba(200,160,50,.6)",fontSize:7,marginTop:4}}>{n.tr}</div>
                      </div>
                    ))}
                  </div>
                ) : null;
              })()}
              <div style={{color:"rgba(200,160,50,.45)",fontSize:8,marginTop:16,lineHeight:1.7}}>Prononce ce Nom dans ton cœur.<br/>Sens sa fréquence résonner en toi.</div>
            </div>
          )}
          {kunStep===4 && (
            <div style={{textAlign:"center",animation:"fadeUp .4s ease both"}}>
              <div style={{fontSize:52,color:"rgba(255,255,150,.9)",fontFamily:"Amiri,serif",filter:"drop-shadow(0 0 20px rgba(255,255,100,.5))",marginBottom:12}}>نور</div>
              <div style={{color:"rgba(255,255,150,.7)",fontSize:10,letterSpacing:3,marginBottom:16}}>L'ONDE DE LUMIÈRE</div>
              <div style={{width:120,height:120,borderRadius:"50%",border:"1px solid rgba(255,255,100,.3)",margin:"0 auto 16px",position:"relative",animation:"pulsOrb 1s ease-in-out infinite"}}>
                <div style={{position:"absolute",inset:10,borderRadius:"50%",border:"1px solid rgba(255,255,100,.5)",animation:"pulsOrb 1s ease-in-out infinite .2s"}}/>
                <div style={{position:"absolute",inset:25,borderRadius:"50%",border:"1px solid rgba(255,255,100,.7)",animation:"pulsOrb 1s ease-in-out infinite .4s"}}/>
                <div style={{position:"absolute",inset:40,borderRadius:"50%",background:"rgba(255,255,150,.9)",animation:"pulsOrb 1s ease-in-out infinite .6s"}}/>
              </div>
              <div style={{color:"rgba(200,160,50,.55)",fontSize:9,lineHeight:1.9}}>L'onde se propage vers les 28 lettres.<br/>Chaque lettre reçoit la vibration.<br/>Le cristal entier s'illumine.</div>
            </div>
          )}
          {kunStep===5 && (
            <div style={{textAlign:"center",animation:"fadeUp .4s ease both"}}>
              <div style={{fontSize:72,color:"white",fontFamily:"Amiri,serif",filter:"drop-shadow(0 0 40px rgba(255,255,255,.8))",marginBottom:16,letterSpacing:4}}>فَيَكُون</div>
              <div style={{color:"rgba(255,255,255,.5)",fontSize:10,letterSpacing:6}}>QUE CELA SOIT</div>
            </div>
          )}

          <button onClick={() => { clearTimeout(kunTimer.current); setKunStep(0); fireKun(input.trim()); }}
            style={{position:"absolute",bottom:24,right:24,background:"none",border:"1px solid rgba(200,160,50,.25)",color:"rgba(200,160,50,.4)",fontSize:7,letterSpacing:2,padding:"5px 10px",cursor:"pointer",fontFamily:"Cinzel,serif",borderRadius:1}}>
            PASSER ▶
          </button>
        </div>
      )}

      <div style={{position:"relative",zIndex:2,marginTop:14,textAlign:"center"}}>
        <div style={{color:"rgba(200,160,50,.45)",fontSize:9,fontFamily:"Amiri,serif",direction:"rtl",marginBottom:4}}>بِسْمِ اللهِ الرَّحْمَٰنِ الرَّحِيمِ</div>
        <div style={{color:"rgba(200,160,50,.25)",fontSize:7,letterSpacing:3,fontFamily:"Amiri,serif",direction:"rtl"}}>١+٥+٦=١٢→٣ · ٣٠+٤٠+٥٠=١٢٠→٣ · هُوَ · كُن</div>
      </div>
    </div>
  );
}
