...
function getRootMeaning(word){
  if(ROOT_MEANINGS[word]) return ROOT_MEANINGS[word];
  ...
  return [...word].map(c=>AR2FR[c]||c).join("-");
}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
                {rootPerms.map((p,i)=>(
                  <div key={i} style={{textAlign:"center",padding:"10px 6px",border:"1px solid rgba(232,201,122,.2)",borderRadius:2,background:"rgba(232,201,122,.03)"}}>
                    ...
                    <div style={{color:"rgba(232,201,122,.55)",fontSize:8,marginTop:3,fontFamily:"Georgia,serif",textAlign:"center",direction:"ltr"}}>{getRootMeaning(p.word)}</div>
                    <div style={{color:W,fontSize:7,marginTop:2}}>{p.value}</div>
                  </div>
                ))}
              </div>
            </div>
