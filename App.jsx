...
const STARS=Array.from({length:110},(_,i)=>({x:(i*137.508)%100,y:(i*97.301)%100,r:.25+(i%5)*.22,dur:3+(i%8),del:-(i%7)}));
...
@keyframes glimmer{0%,100%{opacity:.08}50%{opacity:.55}}
...
export default function NEXUSCrystal() {
  ...
  const [nafarStats,setNafarStats] = useState({});
  ...
  useEffect(() => {
    ...
    socket.on("collective_pulse", ({ word, reduced, archetype, count }) => {
      setNafarCount(count);
      setNafarWord(word||"");
      setNafarPulse(true);
    ...
    socket.on("stats", s => setNafarStats(s));
    socket.emit("get_stats");
    return () => socket.disconnect();
  }, []);

  // ── URL PARAMS : ?kun=word ou ?name=index ──
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const kunParam  = params.get("kun");
    const nameParam = params.get("name");
    if (kunParam) {
      setInput(decodeURIComponent(kunParam));
      setTab("cristal");
    }
    if (nameParam) {
      const idx = parseInt(nameParam, 10);
      if (!isNaN(idx) && N99[idx]) selectName(N99[idx]);
    }
  ...
  const handleKun = useCallback(async () => {
    ...
    // ── NAFAR : émettre activation au serveur ──
    if(socketRef.current) {
      socketRef.current.emit("activate", { word:q, reduced, archetype:ARCH[reduced]||"", name:"" });
    }
    // ── URL SHARE : mettre à jour l'URL sans recharger ──
    const url = new URL(window.location.href);
    url.searchParams.set("kun", encodeURIComponent(q));
        ...
        <g transform={inv ? "translate("+CX*2+",0) scale(-1,1)" : undefined}>
          <g transform={interactive ? "rotate("+rot+","+CX+","+CY+")" : undefined}>
            ...
            {ALL.map(letter => {
              ...
              const fs   = isH?(isV?18:14):(isV?14:11);
              ...
              return (
                <g key={letter.deg}
                  ...
                  {(isH||isV||isLit) && (
                    <circle cx={lx} cy={ly} r={isV?13:10}
                      fill={isLit?"rgba(255,255,100,.12)":isE?"rgba(200,160,50,.13)":isM?"rgba(168,188,220,.1)":"rgba(200,140,40,.1)"}
                      filter={(isV||isLit)?"url(#glow)":undefined}/>
                  )}
                  <text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
                    transform={iTx} fontSize={fs} fill={col}
                    opacity={isH?1:isLit?1:isV?.9:.7}
                  ...
                  {(isV||isH) && (
                    <text x={lx} y={ly+fs/2+9} textAnchor="middle"
                      transform={iTx} fontSize="7" fill={col}
                      opacity={isH?.8:.5}
                  ...
                  {isH && (
                    <text x={lx} y={ly-fs/2-8} textAnchor="middle"
                      fontSize="6.5" fill={col} opacity=".65"
                      style={{fontFamily:"Cinzel,serif",letterSpacing:".5px"}}>
                      {letter.n}
                    </text>
                  )}
                </g>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#020210",display:"flex",flexDirection:"column",alignItems:"center",padding:"0 0 20px",boxSizing:"border-box",position:"relative",overflow:"hidden",fontFamily:"Cinzel,Palatino,serif"}}>
      ...
      {/* ── HEADER ─────────────────────────────────────────── */}
      <div style={{width:"100%",maxWidth:420,padding:"12px 14px 0",boxSizing:"border-box",position:"relative",zIndex:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{color:"rgba(200,165,60,.35)",fontSize:7,letterSpacing:5}}>N E X U S</div>
            <div style={{color:G,fontSize:10,letterSpacing:4,marginTop:2}}>Cristal de HU · هُوَ</div>
          </div>
          <button onClick={() => setShowNames(v=>!v)}
            style={{background:"none",border:"1px solid "+D,color:G,fontSize:7,letterSpacing:2,padding:"5px 9px",cursor:"pointer",fontFamily:"Cinzel,serif",borderRadius:1,marginTop:2}}>
            99 NOMS ▾
          </button>
        </div>
        ...
        {(nafarCount>1||nafarPulse) && (
          <div style={{marginTop:8,padding:"5px 10px",border:"1px solid rgba(200,160,50,.25)",borderRadius:1,background:"rgba(200,160,50,.06)",display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:nafarPulse?"#FFFF80":"rgba(200,160,50,.6)",boxShadow:nafarPulse?"0 0 12px #FFFF80":undefined,transition:"all .3s"}}/>
            <span style={{color:G,fontSize:8,letterSpacing:2}}>NAFAR · {nafarCount} {nafarCount>1?"âmes vibrent":"âme vibre"}</span>
            {nafarWord && <span style={{color:W,fontSize:10,fontFamily:"Amiri,serif",direction:"rtl"}}>{nafarWord}</span>}
          </div>
        )}
      </div>
      ...
      <div style={{position:"relative",zIndex:2,marginTop:14,color:"rgba(180,140,50,.13)",fontSize:7,letterSpacing:4}}>
        ١+٥+٦ · ٣٠+٤٠+٥٠ · هُوَ · كُن
      </div>
    </div>
  );
}
