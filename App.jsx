              ...
              return (
                <g key={letter.deg}
                  ...
                  <text x={lx} y={ly+fs/2+7} textAnchor="middle"
                    transform={iTx} fontSize={isV?"7":"6"} fill={col}
                    opacity={isH?.9:isV?.65:.45}
                    style={{fontFamily:"Cinzel,serif"}}>
                    {letter.v}
                  </text>
                  {/* translittération — toujours visible pour vertex, hover pour arc */}
                  {(isV || isH) && (
                    <text x={lx} y={ly-fs/2-6} textAnchor="middle"
                      transform={iTx} fontSize={isV?"6.5":"6"} fill={col}
                      opacity={isH?.85:isV?.55:.0}
